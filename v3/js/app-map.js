...
(function () {
  ...
  function init() {
    ...
    Promise.all([
      ...
      state.allClassifications = uniqueValues(clients, "clasificacion");
      var defaultOn = ["CLIENTE", "CARTERA (contactada)", "CARTERA (sin contacto)"];
      var initialClasificaciones = state.allClassifications.filter(function (c) { return defaultOn.indexOf(c) !== -1; });
      state.activeClassifications = new Set(initialClasificaciones.length ? initialClasificaciones : state.allClassifications);
  ...
  function zoneTagHtml(zone) {
    if (!zone) return "";
    return '<span class="zone-tag" style="background:' + zone.color + '22;color:' + zone.color + ';border-color:' + zone.color + '">● ' + escapeHtml(zone.name) + "</span>";
  }
  ...
  function popupHtml(c) {
    ...
    return '<div class="popup-card">' +
      ...
      (zone ? '<div class="p-zone">' + zoneTagHtml(zone) + "</div>" : "") +
      '<div class="p-actions">' +
  ...
  function buildFilterUI() {
    ...
    box.innerHTML = "";
    state.allClassifications.slice().sort().forEach(function (c) {
      var chip = document.createElement("div");
      chip.className = "chip" + (state.activeClassifications.has(c) ? " active" : "");
  ...
  function openDetail(id) {
    ...
    if (zone) html += '<div class="d-zone">' + zoneTagHtml(zone) + "</div>";
